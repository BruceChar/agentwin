...
describe('SqliteStorage', () => {
  it('migrates, accounts, balances', async () => {
    const s = createStorage({ engine: 'sqlite', path: ':memory:' });
    await s.init();
    ...
    const got = await s.getAccount(acc.id);
    expect(got?.name).toBe('paper-1');
    await s.close();
  });

  it('upserts and reads klines with filters', async () => {
    const s = createStorage({ engine: 'sqlite', path: ':memory:' });
    await s.init();
    ...
    await s.upsertKlines([{ ...rows[0]!, close: 106 }]); // 更新
  ...
  it('orders, trades, aggregates, positions', async () => {
    ...
    expect(agg.feesPaid).toBeCloseTo(0.22, 5);
    ...
    const pos = await s.getPositions(acc.id);
    expect(pos).toHaveLength(1);
    expect(pos[0]!.avgEntryPrice).toBe(100);
    await s.close();
  });
  ...
  it('strategies, equity, llm, sentiment, backtest, journal', async () => {
    ...
    await s.init();
    const acc = await s.createAccount({ name: 'p', type: 'paper' });
    const strat = await s.createStrategy({ id: 's1', name: 'ma-cross', market: 'SPOT', symbol: 'BTCUSDT', interval: '1h', parameters: { fast: 5, slow: 20 }, source: 'user', enabled: true, createdAt: 1, updatedAt: 1 });
    ...
    expect((await s.listStrategies()).length).toBe(1);
    ...
    const bts = await s.listBacktests();
    expect(bts).toHaveLength(1);
    expect(bts[0]?.result).toEqual({ b: 2 });
    ...
    expect(strat.name).toBe('ma-cross');
    await s.close();
  });
});
